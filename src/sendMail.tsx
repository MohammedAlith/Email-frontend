...
const SendEmails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchEmails = async () => {
      setLoading(true);
      try {
        const res = await fetch("https://emai-node.onrender.com/emails/sent");
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {emails.map((email, idx) => (
            ...
            >
              ...
              <p className="text-sm text-gray-600 mt-2 truncate">To: {email.recipient}</p>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-lg p-6 relative">
            ...
            <p className="text-sm text-gray-500 mb-3">
              {selectedEmail.sent_at && new Date(selectedEmail.sent_at).toLocaleString()}
            </p>
